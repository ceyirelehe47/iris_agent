...
import { isPiArchiveEntryRef } from "@iris/context/contracts";
...
test("r1 gate3: mock slice commits canonical units in order, exactly-once", async () => {
  ...
  try {
    ...
    // runtime-origin 单元：entryId（Pi entryId）唯一（每条消息恰好一个单元）。
    // iris_agent#130 A2：Pi 兼容来源是专用判别 PiArchiveEntryRef（非 DshMessageRef）。
    const sourceIds = new Set(
      units.map((unit) => {
        assert.ok(
          isPiArchiveEntryRef(unit.sourceRef),
          `unit ${unit.unitId} must carry a dedicated PiArchiveEntryRef, got ${JSON.stringify(unit.sourceRef)}`,
        );
        const ref = unit.sourceRef as { entryId?: string };
        return ref.entryId;
      }),
    );
    assert.equal(sourceIds.size, units.length, "entryId must be unique per unit");
...
test("r1 gate4: no synthetic repair — every unit maps to a committed DSH message", async () => {
  ...
  try {
    ...
    // 每个 Pi runtime-origin 单元必须携带稳定的专用 PiArchiveEntryRef
    // （schemaId=iris.pi_archive_entry_ref.v1，entryId 非空、runtimeSessionId
    // 非空）—— 不是 DshMessageRef（iris_agent#130 A2）；无 synthetic 单元
    // （无凭空生成的 assistant/toolResult）。
    for (const unit of units) {
      assert.ok(
        isPiArchiveEntryRef(unit.sourceRef),
        `unit ${unit.unitId} must carry a dedicated PiArchiveEntryRef, got ${JSON.stringify(unit.sourceRef)}`,
      ...
      assert.ok((ref.entryId ?? "").length > 0, `unit ${unit.unitId} Pi entryId must be non-empty`);
      assert.ok(
        (ref.runtimeSessionId ?? "").length > 0,
        `unit ${unit.unitId} Pi runtimeSessionId must be non-empty (archive owner)`,
      );
    }
