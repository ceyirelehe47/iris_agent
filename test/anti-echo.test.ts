...
test("anti-echo: reference_only disposition never becomes evidence basis", () => {
  const unit = unitView({ kind: "user", historianDisposition: "reference_only" });
  assert.equal(isEvidenceEligibleUnit(unit), false);
  assert.equal(toEvidenceBasisRef(unit), undefined);
});

test("anti-echo: exclude disposition never becomes evidence basis", () => {
  const unit = unitView({ kind: "user", historianDisposition: "exclude" });
  assert.equal(isEvidenceEligibleUnit(unit), false);
});

test("anti-echo: classifyEvidenceBasis keeps only eligible units", () => {
  const units = [
    unitView({ contextUnitId: "u1", contextSeq: 1, kind: "user" }),
    unitView({
      contextUnitId: "u2",
      contextSeq: 2,
      kind: "assistant",
    ...
    unitView({ contextUnitId: "u3", contextSeq: 3, kind: "tool_result" }),
    unitView({
      contextUnitId: "u4",
      contextSeq: 4,
      kind: "user",
