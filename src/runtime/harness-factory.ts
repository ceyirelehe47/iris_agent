...
// ContextGeneration (unversioned domain name) via the Provider Renderer; the
// m0/m1 ContextRenderer is gone (Notion 01 Context Assembly｜Provider Wire
...
export function createIrisHarness(options: CreateIrisHarnessOptions): {
  ...
  const harness = new AgentHarness({
    ...
    contextController: async () => {
      ...
      if (generation === null) {
        throw new Error(
          "IRIS_CONTEXT_TRANSFORM_UNAVAILABLE: no validated ContextGeneration at provider boundary",
        );
      }
